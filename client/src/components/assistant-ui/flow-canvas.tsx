...
export function FlowCanvas({
  ...
  return (
    ...
    >
      <svg
        aria-hidden
        className="aui-flow-canvas-edges pointer-events-none absolute inset-0 z-0 h-full w-full overflow-visible text-muted-foreground/70"
        data-slot="flow-canvas-edges"
      >
        {rendered.map((edge, index) => (
          <g data-slot="flow-canvas-edge" key={index}>
            <path
              d={edge.path}
              fill="none"
              stroke="currentColor"
      ...
      <div className="relative z-10" data-slot="flow-canvas-content">
        {children}
      </div>
      {rendered.map(
        (edge, index) =>
          edge.label && (
            <span
              className="aui-flow-canvas-label absolute z-20 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap bg-background px-1.5 text-muted-foreground text-xs"
