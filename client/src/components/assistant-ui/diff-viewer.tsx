...
function DiffViewerStats({
	...
	return (
		<span className="flex gap-2 text-xs" data-slot="diff-viewer-stats">
...
function DiffViewerFile({ className, ...props }: ComponentProps<"div">) {
	return (
		<div className={cn(className)} data-slot="diff-viewer-file" {...props} />
	);
}
	...
	return (
		...
		>
			{parsedFiles.map((file, fileIndex) => (
				<div data-slot="diff-viewer-file" key={fileIndex}>
					<DiffViewerHeader
						...
						showStats={showStats}
					/>
					<div className="overflow-x-auto" data-slot="diff-viewer-content">
						{viewMode === "split"
