...
const hash = (n: number, salt: number, range: number) => {
	let h = (Math.imul(n, 374_761_393) + Math.imul(salt, 668_265_263)) >>> 0;
		...
		>
			...
			>
				{DOT_INDEXES.map((i) => {
					...
					return (
						<circle
							className="duration-300 [animation-iteration-count:infinite] [animation-name:aui-dot-matrix-blink] [animation-timing-function:ease-in-out] [transition-property:--aui-dot-matrix-hi,--aui-dot-matrix-lo,opacity] motion-reduce:[animation-name:none]"
							cx={2 + col * 4}
							cy={2 + row * 4}
